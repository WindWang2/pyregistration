...
from scipy.ndimage.filters import gaussian_filter, convolve, correlate
from scipy.fftpack import fftshift
from scipy.ndimage.filters import gaussian_gradient_magnitude
double = float
__author__ = "Pedro Garcia Freitas <sawp@sawp.com.br>; Add some functions by Wind_Wang"
__version__ = "$Revision: 0.31 $"
__date__ = "$Date: 2014/11/20 $"
__copyright__ = "Copyright (c) 2011 Pedro Garcia"
__license__ = "GPLv2"

def std(image):
    """Computes the Standard Deviation of Main Image (No Reference).
...
def sf(image):
    """Computes the Spatial Frequency of the Image (No Reference)
    ...
    def calfreq(data):
        square = data**2
        fre = sqrt(square.mean())
        return fre
    rfdata = img[:,:-1]-img[:,1:]
    cfdata = img[:-1,:]-img[1:,:]
    SF = sqrt(calfreq(rfdata)**2+calfreq(cfdata)**2)
    return SF

def ag(image):
    """Computes the Average Gradient of the Image (No Reference)
    ...
    """
    img = image.astype('double')
    rfdata = img[:-1,:-1]-img[:-1,1:]
    cfdata = img[:-1,:-1]-img[1:,:-1]
    (x,y) = img.shape
    AG = sum(sqrt((cfdata**2+rfdata**2)/2))/((x-1)*(y-1))
    return AG 

def agg(image):
    """Computes the Gaussian Gradient of the Image (No Reference)
    ...
    """
    img = image.astype('double')
    Agg = sum(gaussian_gradient_magnitude(img,0.5))/(img.size)
    return Agg

def entropy(image):
    """Computes the Entropy of image(No Reference)
    
       Value = entropy(reference)
       ...
       Note
       ----------
       Only for 8-bit Image
    """
    img = image.astype('double')
    refHist,refTemp = histogram(img,bins = 255,range=(0,255))
    refHist = refHist/double(img.size)
    re = -(refHist*log2(refHist+0.00000000000000000001))
    re[refHist == 0] = 0
    entropy = sum(re)
    return entropy  
...
def mse(reference, query):
    ...
    mean = square.mean()
    return mean
def nlse(reference, query):
    """Computes the Normalised Least Square Error(NLSE) of two images.

    value = NLSE(reference, query)
    ...
    square = (diff ** 2)
    sqref = ref**2
    nlse = sum(square)/(sum(sqref))
    return nlse 

def rmse(reference, query):
    """Computes the Root of Mean square Error of two images.

    value = RMSE(reference, query)
    ...
    value    : RMSE value
    """
    return sqrt(mse(reference,query)) 

def rm(reference, query):
    """Computes the Relative shift in mean(RM) of two images.

    value = rm(reference, query)
    ...
    (ref, que) = (reference.astype('double'), query.astype('double'))
    diff =abs(ref - que)
    rmbyPix= (diff / (ref+0.00000000000000000000001))
    rmbyPix[reference == 0]=0
    RM = rmbyPix.mean()
    return RM 

def cc(reference, query):
    """Computes the Correlation coefficient of two images.

    value = cc(reference, query)
    ...
    ref.shape = size(ref)
    que.shape = size(que)
    CC = corrcoef(ref,que)[0,1]
    return CC

def ce(reference, query):
    """Computes the Cross entropy of two images.

    value = ce(reference, query)
    ...
    (ref, que) = (reference.astype('double'), query.astype('double'))
    refHist,refTemp = histogram(ref,bins = 255,range=(0,255))
    queHist,queTemp = histogram(que,bins = 255,range=(0,255))
    refHist=refHist/double(size(ref))
    queHist=queHist/double(size(que))

    re = -queHist*log2(abs((queHist)/(refHist+eps))+eps)
    re[refHist == 0]=0
    CE = sum(re)
    return CE
...
def psnr(reference, query, normal=255):
    ...
    if msev != 0:
        value = 10.0 * log10(normalization * normalization / msev)
    else:
        value = float("inf")
    return value

def nmi(reference, query):
    """Computes the Normalized mutual information (NMI).

    value = nmi(reference, query)
...
def snr(reference, query):
    ...
    msev = mse(reference, query)
    if msev != 0:
        value = 10.0 * log10(signal_value / msev)
    else:
        value = float("inf")
    return value
...
def ssim(reference, query):
    ...
    def __get_mus(img1, img2):
        (mu1, mu2) = map(lambda x: gaussian_filter(x, 1.5), (img1, img2))
        (m1m1, m2m2, m1m2) = (mu1 * mu1, mu2 * mu2, mu1 * mu2)
        return (m1m1, m2m2, m1m2)

    def __get_sigmas(img1, img2, m1m1, m2m2, m1m2):
        s1 = gaussian_filter(img1 * img1, 1.5) - m1m1
        s2 = gaussian_filter(img2 * img2, 1.5) - m2m2
        s12 = gaussian_filter(img1 * img2, 1.5) - m1m2
        return (s1, s2, s12)
    ...
    def __get_negative_ssimap(C1, C2, m1m2, m11, m22, s12, s1s1, s2s2):
        ...
        ssim_map = ones(img1.shape)
        indx = (den1 * den2 > 0)
        ssim_map[indx] = (num1[indx] * num2[indx]) / (den1[indx] * den2[indx])
        indx = bitwise_and(den1 != 0, den2 == 0)
...
def uqi(reference, query):
    ...
    """
    def __conv(x):
        return convolve(x, ones((BLOCK_SIZE, BLOCK_SIZE)))
    ...
    def __get_quality_map(b1, b2, b3, b4, b5, b6, b7, BLOCK_SIZE):
        ...
        index = bitwise_and(denominator1 == 0, b7 != 0)
        quality_map = ones(denominator.shape)
...
def pbvif(reference, query):
    ...
    def __get_num(s1s1, sv_sq, sigma_nsq, g):
        ...
        snr = log10(1.0 + normg).sum()
        return snr

    def __get_den(s1s1, sigma_nsq):
        snr = log10(1.0 + s1s1 / sigma_nsq)
        return snr.sum()
...
def mssim(reference, query):
    ...
    """
    def __get_filt_kern():
        num = matrix([131, -199, -101, 962, 932, 962, -101, -199, 131]).T
        den = matrix([3463, 8344, 913, 2549, 1093, 2549, 913, 8344, 3463]).T
    ...
    def __get_MVR(img1, img2, K, nlevs):
        (ssim_v, ssim_r) = (zeros((nlevs, 1)), zeros((nlevs, 1)))
    ...
    def __calc_mssim_mvr(img1, img2):
        ...
        (alpha, beta, lvl) = (0.1333, matrix(weights).T, len(weights))
    ...
    ssim_mvr = matrix(__calc_mssim_mvr(ref, quer))
    value = ssim_mvr.prod()
    return value
...
def __get_gaussian_kernel(N=15, sigma=1.5):
    (H, W) = ((N - 1) / 2, (N - 1) / 2)
    std = sigma
    (y, x) = mgrid[-H:H + 1, -W:W + 1]
    arg = -(x * x + y * y) / (2.0 * std * std)
    h = exp(arg)
    index = h < finfo(float).eps * h.max(0)
    h[index] = 0
    sumh = h.sum()
    if sumh != 0:
        h = h / sumh
...
def __ssim_modified(reference, query, K):
    ...
    def __get_sigmas(img1, img2, window, m1m1, m2m2, m1m2):
        ...
        (s1, s2) = map(sqrt, (abs(s1s1), abs(s2s2)))
    ...
    def __MVR_neg_kern(m, s, s_square):
        ...
        def __calcR(mu1, s1, s2, s12):
            ...
            index_s2 = index1 * logical_not(index2) > 0
            R[index_s2] = 0.0
            return R
...
def nqm(reference, query):
    ...
    def __decompose_cos_log_filter(w1, w2, phase=pi):
        return 0.5 * (1 + cos(pi * log2(w1 + w2) - phase))
    ...
    def __get_u(r):
        u = [4 * (logical_not((r + 2 <= 4) * (r + 2 >= 1)))]
        u += [4 * (logical_not((r <= 4) * (r >= 1)))]
        u += [0.5 * (logical_not((r >= 2) * (r <= 8)))]
        u += [4 * (logical_not((r >= 4) * (r <= 16)))]
        u += [0.5 * (logical_not((r >= 8) * (r <= 32)))]
        u += [4 * (logical_not((r >= 16) * (r <= 64)))]
        return u

    def __get_G(r):
        (w, u) = (__get_w(r), __get_u(r))
        phase = [pi, pi, 0.0, pi, 0.0, pi]
    ...
    def __compute_fft_plane_shifted(ref, query):
        (x, y) = ref.shape
        (xplane, yplane) = mgrid[-y / 2:y / 2, -x / 2:x / 2]
        plane = (xplane + 1.0j * yplane)
        r = abs(plane)
        G = __get_G(r)
        Gshifted = map(fftshift, G)
    ...
    def __get_detection_thresholds():
        viewing_angle = (1.0 / 3.5) * (180.0 / pi)
    ...
    def __reconstruct_images(A, AI):
        return map(lambda x: add.reduce(x), (A, AI))
    ...
    (ref, query) = map(lambda x: x.astype('double'), (reference, query))
    (A, AI) = __get_correlated_images(ref, query)
...
def wsnr(reference, query):
    ...
    def __create_complex_planes(x, y):
        ...
        (xplane, yplane) = mgrid[-H:H + 1, -W:W + 1]
        return (xplane, yplane)

    def __get_evaluated_contrast_sensivity(plane):
        w = 0.7
        return ((1.0 - w) / 2.0) * cos(4.0 * angle(plane)) + (1.0 + w) / 2.0
    ...
    def __generate_CSF(radfreq):
        a = -((0.114 * radfreq) ** 1.1)
        csf = 2.6 * (0.0192 + 0.114 * radfreq) * exp(a)
        f = radfreq < 7.8909
        csf[f] = 0.9809
        return csf

    def __find_wighted_SNR_in_frequency_domain(csf):
        err = reference.astype('double') - query.astype('double')
        err_wt = fftshift(fft2(err)) * csf
        im = fft2(reference)
        return (err, err_wt, im)

    def __get_weighted_error_power(err_wt):
        return (err_wt * conj(err_wt)).sum()

    def __get_signal_power(im):
        return (im * conj(im)).sum()

    def __get_ratio(mss, mse):
        if mse != 0:
            ratio = 10.0 * log10(mss / mse)
        else:
            ratio = float("inf")
        return real(ratio)
    ...
    (err, err_wt, im) = __find_wighted_SNR_in_frequency_domain(csf)
